...
uint8_t runSOCDCleaner(SOCDMode mode, uint8_t dpad)
{
	...
	switch (dpad & (GAMEPAD_MASK_UP | GAMEPAD_MASK_DOWN))
	{
		case (GAMEPAD_MASK_UP | GAMEPAD_MASK_DOWN):
			...
			else if ((mode == SOCD_MODE_SECOND_INPUT_PRIORITY || mode == SOCD_MODE_NEUTRAL_LR_LAST_UD) && lastUD != DIRECTION_NONE)
	...
	switch (dpad & (GAMEPAD_MASK_LEFT | GAMEPAD_MASK_RIGHT))
	{
		case (GAMEPAD_MASK_LEFT | GAMEPAD_MASK_RIGHT):
			if (mode == SOCD_MODE_NEUTRAL_LR_LAST_UD)
				lastLR = DIRECTION_NONE;
			else if (mode == SOCD_MODE_SECOND_INPUT_PRIORITY && lastLR != DIRECTION_NONE)
